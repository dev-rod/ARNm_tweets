...
get_score <- function(text) {
    return(syuzhet::get_sentences(text) %>%
        syuzhet::get_sentiment(method="nrc", language="french") %>%
        sum())
}
...
{
    tweets <- read_csv(filename, col_types = cols())
    ...
    # On ne garde que les tweets associés à la recherche arnm
    # uniquement les colonnes qui nous interesse
    # en supprimant les doublons que query
    # en affectant le score de sentiment
    tweets_arnm <- tweets %>%
        filter(query %in% (c("\"arn messager\"", "\"arn messager\" OR ARNm", "vaccin AND (pfizer OR biontech OR moderna OR ARNm)"))) %>%
        select(c(1:5, 8:9, 12, 14, 17, 30, 48, 54, 73:75, 78, 79, 91)) %>%
